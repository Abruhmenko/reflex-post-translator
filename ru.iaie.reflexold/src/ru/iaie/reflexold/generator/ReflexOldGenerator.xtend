...
class ReflexOldGenerator implements IReflexOldGenerator {
	...
	override void beforeGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
		...
		fileName = '''«GENERATED_DIR_NAME»/«generatorHelper.translateName(program.name)».post'''
	}
	...
	def generateInputVars() {
		return '''
		«FOR process : program.processes»
		«FOR procVar : process.variables»
		«IF procVar.isPhysical && (procVar as PhysicalVariable).isInputVar»
		«generatorHelper.generateProcVarDefinition(procVar)»
		«ENDIF»
		«ENDFOR»
		«ENDFOR»
		'''
	...
	def generateOutputVars() {
		return '''
		«FOR process : program.processes»
		«FOR procVar : process.variables»
		«IF procVar.isPhysical && (procVar as PhysicalVariable).isOutputVar»
		«generatorHelper.generateProcVarDefinition(procVar)»
		«ENDIF»
		«ENDFOR»
		«ENDFOR»
	...
	def generateGlobalVars() {
		...
		«FOR process : program.processes»
		«FOR procVar : process.variables»
		«IF procVar.isShared && !procVar.isPhysical»
		«generatorHelper.generateProcVarDefinition(procVar)»
		«ENDIF»
		«ENDFOR»
		«ENDFOR»
	...
	def generateEnumVars() {
		return '''
		«FOR enumerator : program.enums»
		«generatorHelper.generateEnumVarDefinition(enumerator)»
		«ENDFOR»
		'''
	}
	
	def generateConstVars() {
		return '''
