...
class ReflexOldModelUtil {
	...
	*/
	...
	def static boolean isInputVar(PhysicalVariable v) {
		return (v.mapping.port.type == PortType.INPUT_EN || v.mapping.port.type == PortType.INPUT_RU)
	}
	
	def static boolean isOutputVar(PhysicalVariable v) {
		return (v.mapping.port.type == PortType.OUTPUT_EN || v.mapping.port.type == PortType.OUTPUT_RU)
	}
	...
	def static boolean isCaseStatLast(CaseStat option, SwitchStat s) {
		return s.options.indexOf(option) == (s.options.length - 1)
	}
