...
public class ReflexOldModelUtil {
  ...
  public static boolean isInputVar(final PhysicalVariable v) {
    return (Objects.equal(v.getMapping().getPort().getType(), PortType.INPUT_EN) || Objects.equal(v.getMapping().getPort().getType(), PortType.INPUT_RU));
  }
  
  public static boolean isOutputVar(final PhysicalVariable v) {
    return (Objects.equal(v.getMapping().getPort().getType(), PortType.OUTPUT_EN) || Objects.equal(v.getMapping().getPort().getType(), PortType.OUTPUT_RU));
  }
  ...
  public static boolean isCaseStatLast(final CaseStat option, final SwitchStat s) {
    int _indexOf = s.getOptions().indexOf(option);
    int _length = ((Object[])Conversions.unwrapArray(s.getOptions(), Object.class)).length;
    int _minus = (_length - 1);
    return (_indexOf == _minus);
  }
